  ...
  return (
    <section className="py-12 px-6 bg-gray-100">
      <div className="container mx-auto max-w-4xl">
        <div className={`flex flex-col ${reverse ? 'md:flex-row-reverse' : 'md:flex-row'} md:space-x-8 md:space-x-reverse`}>
          {/* Imagem */}
          ...
          <div className="flex-1">
            <h2 className="text-3xl font-semibold mb-4 text-[rgb(17,24,39)]">
              {title}
            </h2>
            {content.map((paragraph, index) => (
              <p key={index} className="text-lg mb-4 text-[rgb(75,85,99)]">
                {paragraph}
              </p>
            ))}
          </div>
